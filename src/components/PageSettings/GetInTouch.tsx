...
import { GetInTouchPageSchema } from "@/helper/schema";
...
const GetInTouch = () => {
  ...
  const {
    ...
    url: `page-setting/info?name=get_in_touch`,
    method: "client",
  });
  ...
  const {
    ...
    resolver: yupResolver(GetInTouchPageSchema),
  });
  ...
  const onGetInTouchPageSubmit: SubmitHandler<{
    header_text: string;
    ...
    try {
      const res = await post(
        `page-setting/get-in-touch-page`,
        formData,
        "multipart/form-data"
      );
  ...
  return (
    <div className="flex flex-col gap-[35px]">
      ...
      <form
        onSubmit={handleSubmit(onGetInTouchPageSubmit)}
